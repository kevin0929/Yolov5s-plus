...
def run(
    ...
    space_model = spaceregression.build_space_model()
    # Run tracking
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], det[:, 4])):
                        ...
                        #mid_p_x = (bboxes[0] + bboxes[2]) / 2
                        #mid_p_y = (bboxes[1] + bboxes[3]) / 2
                        #pred_list = space_model.predict(np.array([[mid_p_x, mid_p_y]]))
                        #pred_space_x = pred_list[0][0]
                        #pred_space_y = pred_list[0][1]
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            #label = label + " " + "(" + str(np.around(pred_space_x, 1)) + "," + str(np.around(pred_space_y, 1)) + ")"
